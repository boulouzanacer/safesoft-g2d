...
<div class="space-y-6">
    <div class="store-panel p-4 sm:p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                <div class="text-2xl font-extrabold tracking-wide">{{ __('Tous les produits') }}</div>
                <div class="store-muted mt-1 text-sm">{{ __('Recherche produit avec filtre par catégorie de boutique.') }}</div>
            </div>
        ...
        <form method="GET" action="{{ url('/produits') }}" class="mt-5 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-3">
            <div class="relative">
                ...
                <input name="q"
                       value="{{ $q }}"
                       placeholder="{{ __('Rechercher un produit ou une boutique...') }}"
        ...
        @include('store.partials.boutique-category-grid', [
            ...
            'subtitle' => __('Choisissez une catégorie de boutique pour filtrer les produits de façon plus visuelle.'),
        ])
    </div>
    ...
    <div class="space-y-3">
        <div class="flex items-center justify-between">
            ...
            <div class="store-muted text-sm">{{ $produits->total() }} {{ __('produit(s)') }}</div>
        </div>
