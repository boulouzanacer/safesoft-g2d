...
<head>
    ...
    <title>{{ __($title ?? 'Administration') }} - {{ config('branding.platform_name') }}</title>
    ...
    <style>
        ...
        [dir="rtl"] .admin-profile-menu{
            right:auto;
            left:0;
        }
        [dir="rtl"] .admin-profile-text,
        [dir="rtl"] .admin-text-left{
            text-align:right;
        }
        .keep-ltr{
            direction:ltr;
...
<div class="flex min-h-screen">
    <aside class="admin-sidebar fixed inset-y-0 left-0 w-[260px] border-r bg-[var(--admin-bg)]"
        ...
        <div class="h-16 px-6 flex items-center gap-3 border-b"
            ...
            <div class="leading-tight">
                ...
                <div class="text-xs" :class="dark ? 'text-white/60' : 'text-slate-500'">{{ __('Administration') }}</div>
            </div>
        </div>
        ...
        <nav class="px-4 py-4 space-y-1 text-sm" :class="dark ? 'text-slate-100' : 'text-slate-900'">
            ...
            <a href="{{ url('/admin/dashboard#boutique-categories-section') }}"
               ...
               :class="dark ? 'hover:bg-white/10' : 'hover:bg-slate-100'">
                ...
                <span>{{ __('Catégories Boutiques') }}</span>
            </a>
            ...
            <form method="POST" action="{{ url('/admin/logout') }}" class="pt-2">
                @csrf
                <button type="submit"
                        class="admin-text-left w-full flex items-center gap-3 rounded-xl px-4 py-3 text-left border border-transparent"
                        :class="dark ? 'hover:bg-white/10' : 'hover:bg-slate-100'">
    ...
    <div class="admin-main flex-1 ml-[260px]">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="relative" @click.outside="profileOpen = false">
                    <button type="button"
                        ...
                        <div class="admin-profile-text text-left leading-tight hidden sm:block">
                            <div class="text-sm font-bold">{{ ($admin?->prenom ?? 'Admin').' '.($admin?->nom ?? '') }}</div>
                            <div class="text-xs opacity-70">{{ __('Admin') }}</div>
                        </div>
                    ...
                    <div x-show="profileOpen"
                         x-transition
                         class="admin-profile-menu absolute right-0 mt-2 w-48 rounded-xl border border-white/10 shadow-2xl overflow-hidden"
                        ...
                        <form method="POST" action="{{ url('/admin/logout') }}">
                            @csrf
                            <button type="submit"
                                    class="admin-text-left w-full text-left px-4 py-3 text-sm hover:bg-white/10"
                                    :class="dark ? '' : 'hover:bg-slate-100'">
                                {{ __('Déconnexion') }}
                            </button>
                        </form>
