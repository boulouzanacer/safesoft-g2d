...
<div class="max-w-7xl space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="rounded-3xl border border-sky-400/20 bg-gradient-to-br from-sky-500/20 to-sky-500/5 p-5 shadow-lg shadow-sky-950/10">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <div class="text-xs font-bold uppercase tracking-[0.2em] text-sky-200/80">{{ __('Achat Client') }}</div>
            ...
            <div class="mt-3 text-xs text-sky-100/70">{{ __('Total purchases for this client with this boutique.') }}</div>
        </div>
        ...
        <div class="rounded-3xl border border-emerald-400/20 bg-gradient-to-br from-emerald-500/20 to-emerald-500/5 p-5 shadow-lg shadow-emerald-950/10">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <div class="text-xs font-bold uppercase tracking-[0.2em] text-emerald-200/80">{{ __('Versement Client') }}</div>
            ...
            <div class="mt-3 text-xs text-emerald-100/70">{{ __('Total payments received from this client.') }}</div>
        </div>

        <div class="rounded-3xl border bg-gradient-to-br p-5 shadow-lg {{ $soldeBadge }}">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <div class="text-xs font-bold uppercase tracking-[0.2em] text-white/70">{{ __('Solde Client') }}</div>
            ...
            <div class="mt-3 text-xs text-white/70">
                {{ $soldeClient > 0 ? __('Outstanding balance to collect.') : ($soldeClient < 0 ? __('Client has credit available.') : __('Account is balanced.')) }}
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-2 rounded-3xl border border-white/10 bg-[var(--frs-card)] p-5 md:p-6">
            <div class="flex items-center justify-between gap-3">
                <div class="font-extrabold tracking-wide">{{ __('Client Information') }}</div>
                <div class="text-xs text-white/50">{{ __('Structured overview') }}</div>
            </div>

            <div class="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Code Client') }}</div>
                    <div class="keep-ltr mt-2 font-extrabold break-words">{{ $client->code_client ?: '-' }}</div>
                </div>
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Telephone') }}</div>
                    <div class="keep-ltr mt-2 font-extrabold break-words">{{ $client->telephone ?: '-' }}</div>
                </div>
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Type') }}</div>
                    <div class="mt-2 font-extrabold capitalize">{{ $client->type_client ?: '-' }}</div>
                </div>
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Tarif') }}</div>
                    <div class="mt-2 font-extrabold">PV {{ (int) ($client->tarif ?? 1) }}</div>
                </div>
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Prevendeur') }}</div>
                ...
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4 md:col-span-2">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Adresse') }}</div>
    ...
    <div class="rounded-3xl border border-white/10 bg-[var(--frs-card)] p-5 md:p-6">
        <div class="flex items-center justify-between gap-3">
            <div>
                <div class="font-extrabold tracking-wide">{{ __('Programme tournee du client') }}</div>
                <div class="text-sm text-white/50">{{ __('Si active, la generation automatique de minuit remplira les 60 prochains jours pour ce client.') }}</div>
            </div>
            <a href="{{ url('/fournisseur/visites/planning') }}"
               class="rounded-2xl px-4 py-3 text-sm font-bold border border-white/10 hover:bg-white/10">
                {{ __('Voir les tournees') }}
            </a>
        </div>
        ...
        <form method="POST" action="{{ url('/fournisseur/clients/'.$client->id.'/planning') }}" class="mt-5 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <label for="programme_tournee_actif" class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Programme tournee') }}</label>
                    <div class="mt-3 flex items-center gap-3">
                        ...
                        <span class="font-extrabold">{{ $programmeTourneeActif ? __('Active') : __('Desactive') }}</span>
                    </div>
                    @error('programme_tournee_actif') <div class="mt-2 text-sm text-red-300">{{ $message }}</div> @enderror
                </div>
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <label class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Type') }}</label>
                    <select name="frequency_type" class="mt-3 w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                        <option value="daily" @selected(old('frequency_type', $activePlan?->frequency_type) === 'daily')>{{ __('Quotidien') }}</option>
                        <option value="weekly" @selected(old('frequency_type', $activePlan?->frequency_type) === 'weekly')>{{ __('Hebdomadaire') }}</option>
                        <option value="monthly" @selected(old('frequency_type', $activePlan?->frequency_type) === 'monthly')>{{ __('Mensuel') }}</option>
                    </select>
                    @error('frequency_type') <div class="mt-2 text-sm text-red-300">{{ $message }}</div> @enderror
                </div>
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <label class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Intervalle') }}</label>
                ...
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <label class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Occurrence mensuelle') }}</label>
            ...
            <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Jours de visite') }}</div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="text-sm text-white/60">
                    {{ __('Details actuels') }}: {{ $activePlan?->frequency_type ? __($activePlan->frequency_type) : '-' }} | {{ __('intervalle') }} {{ $activePlan?->interval_value ?: '-' }} | {{ __('jours') }} {{ $planningDays !== '' ? $planningDays : '-' }}
                </div>
                <button type="submit"
                        class="rounded-2xl px-5 py-3 font-bold text-white"
                        style="background: linear-gradient(135deg, var(--frs-primary), #0A3D7A);">
                    {{ __('Enregistrer le programme tournee') }}
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="rounded-3xl border border-white/10 bg-[var(--frs-card)] overflow-hidden">
        <div class="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="font-extrabold tracking-wide">{{ __('Order History') }}</div>
                <div class="text-sm text-white/50">{{ __('Recent commandes linked to this client.') }}</div>
            </div>
            <a href="{{ url('/fournisseur/commandes?client='.$client->id) }}"
               class="text-sm font-bold text-[var(--frs-primary)] hover:opacity-90">
                {{ __('Open full commandes list') }}
            </a>
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="text-white/60">
                    <tr>
                        ...
                        <th class="text-left py-3 px-4 font-semibold">{{ __('Status') }}</th>
                        <th class="text-right py-3 px-4 font-semibold">{{ __('Amount') }}</th>
                        <th class="text-right py-3 px-4 font-semibold">{{ __('Action') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/10">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-10 text-center text-white/60">{{ __('No orders found for this client.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
