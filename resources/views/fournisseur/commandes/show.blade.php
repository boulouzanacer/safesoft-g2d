    ...
    <div class="rounded-3xl border border-white/10 bg-[var(--frs-card)] p-5 md:p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex items-start gap-4">
                ...
                <div class="min-w-0">
                    <div class="text-2xl font-extrabold tracking-wide break-words">{{ __('Commande') }} #{{ $commande->id }}</div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-2 rounded-3xl border border-white/10 bg-[var(--frs-card)] p-5 md:p-6">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div class="font-extrabold tracking-wide">{{ __('Client & Livraison') }}</div>
                @if((int)($client?->id ?? 0) > 0)
                    <a href="{{ url('/fournisseur/clients/'.$client->id) }}"
                       class="inline-flex items-center gap-2 text-sm font-bold text-[var(--frs-primary)] hover:opacity-90">
                        <span>{{ __('Ouvrir fiche client') }}</span>
            ...
            <div class="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Nom') }}</div>
                ...
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Email') }}</div>
                ...
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4 md:col-span-2">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Adresse livraison') }}</div>
                ...
                <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Téléphone livraison') }}</div>
                ...
                @if(trim((string)($commande->notes ?? '')) !== '')
                    <div class="rounded-2xl border border-white/10 bg-black/20 p-4 md:col-span-2">
                        <div class="text-xs font-bold uppercase tracking-wide text-white/50">{{ __('Notes') }}</div>
        ...
        <div class="rounded-3xl border border-white/10 bg-[var(--frs-card)] p-5 md:p-6">
            <div class="font-extrabold tracking-wide">{{ __('Statut') }}</div>
            ...
            <div class="mt-5 space-y-3">
                ...
                @if($current === 'annulee')
                    <div class="flex items-center gap-3 rounded-2xl border border-red-400/20 bg-red-500/10 px-4 py-3">
                        ...
                        <div class="font-extrabold text-red-200">{{ __('annulee') }}</div>
                    </div>
                @endif
            </div>
                ...
                <div class="flex flex-col sm:flex-row gap-2">
                    <select name="statut"
                        ...
                        @foreach($statuts as $s)
                            <option value="{{ $s }}" @selected($current === $s)>{{ __($s) }}</option>
                        @endforeach
                    </select>
                    <button type="submit"
    ...
    <div class="rounded-3xl border border-white/10 bg-[var(--frs-card)] overflow-hidden">
        <div class="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <div class="text-sm text-white/50">{{ $lignes->count() }} {{ __('ligne(s)') }}</div>
            </div>
            <div class="text-sm font-extrabold text-white/80">{{ __('Total') }} <span class="keep-ltr-inline">{{ number_format((float)$commande->montant_total, 2, '.', ' ') }}</span></div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead class="text-white/60">
                    <tr>
                        <th class="text-left py-3 px-4 font-semibold">{{ __('Produit') }}</th>
                        <th class="text-right py-3 px-4 font-semibold">{{ __('Qté') }}</th>
                        <th class="text-right py-3 px-4 font-semibold">{{ __('Prix unit') }}</th>
                        <th class="text-right py-3 px-4 font-semibold">{{ __('Sous-total') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/10">
                    @foreach($lignes as $l)
                        <tr class="hover:bg-white/5">
                            <td class="py-3 px-4">
                                <div class="font-semibold break-words">{{ $l->produit_designation ?? __('Produit') }}</div>
                ...
                <tfoot>
                    <tr class="border-t border-white/10">
                        <td colspan="3" class="py-4 px-4 text-right font-extrabold">{{ __('Total') }}</td>
