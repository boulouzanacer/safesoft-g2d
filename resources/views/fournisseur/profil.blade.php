...
<div class="max-w-4xl space-y-4">
    ...
    <div class="rounded-2xl border border-white/10 bg-[var(--frs-card)] p-6">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <div class="text-2xl font-extrabold tracking-wide">Domaines personnalisés</div>
                <div class="mt-1 text-sm text-white/60">
                    Branche un domaine comme <span class="font-mono text-white/80">www.boutika.com</span> pour afficher la boutique sans montrer <span class="font-mono text-white/80">g2d-dz.com</span>.
                </div>
            </div>
        ...
        <form method="POST" action="{{ url('/fournisseur/profil/custom-domains') }}" class="mt-5 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-3">
            @csrf
            <div>
                <label class="block text-sm font-semibold text-white/70 mb-1">Nouveau domaine</label>
                <input name="domain"
                       value="{{ old('domain') }}"
                       placeholder="www.boutika.com"
                       class="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 outline-none focus:border-[var(--frs-primary)]">
                <div class="mt-1 text-xs text-white/50">
                    Tu peux saisir seulement le domaine, sans <span class="font-mono">https://</span>.
                </div>
            </div>
        ...
        <div class="mt-6 space-y-3">
            @forelse($frs->customDomains as $domain)
                    ...
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div class="min-w-0">
                            ...
                            <div class="mt-2 text-xs text-white/50">
                                Quand ce domaine pointera vers G2D, la boutique s’ouvrira directement dessus.
                            </div>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            @if(! $domain->is_primary)
                                <form method="POST" action="{{ url('/fournisseur/profil/custom-domains/'.$domain->id.'/primary') }}">
                                    @csrf
                                    <button type="submit"
                                            class="rounded-2xl px-4 py-2 text-sm font-extrabold border border-white/10 hover:bg-white/10">
                                        Définir principal
                            ...
                            @endif
                            <form method="POST" action="{{ url('/fournisseur/profil/custom-domains/'.$domain->id) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
